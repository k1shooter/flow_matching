...
class TrainLogger:
    def __init__(self, log_dir: Path, rank: int, cfg: bool = False):
        ...
        self.enable_wandb = (
            self.cfg.logging.enable_wandb and (rank == 0) and (wandb is not None)
        )

        if self.cfg.logging.enable_wandb and (rank == 0) and (wandb is None):
            self._logger.warning(
                "wandb is not installed; proceeding with text logging only."
            )
