...
from data.data import DataState
...
class TrainState:
    ...
    def restore_checkpoint(
        ...
        if ckpt_dir.exists():
            ...
            self.step = int(loaded_state["step"])
        ...
        if rank == 0:
            logging.warning(f"No checkpoint found at {ckpt_dir}. Starting fresh.")

    def save_checkpoint(self, ckpt_dir: Path, rank: int) -> None:
        if rank != 0:
            return
